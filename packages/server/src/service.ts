...
export const IntrospectionCallingType = ObjectType({
...
export const CallingSuccessType = ObjectType({
...
export const CallingFailedType = ObjectType({
...
export const SingleCallOutputType = Union(CallingSuccessType, CallingFailedType)
export type SingleCallOutput = ToType<typeof SingleCallOutputType>

export const BatchOutputType = ObjectType({
  kind: Literal('Batch' as const),
  ...
  try {
    ...
    if (inputValidateResult === true) {
      try {
        ...
        result = serialize(
          CallingFailed(new ConvertError(err.message, kind(CallingType)))
        )
      }
    } else {
