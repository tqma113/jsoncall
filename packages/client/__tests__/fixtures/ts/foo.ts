...
import { createJSONCall } from '../../../src'
...
export const createCallCreators = <I, II>(fooDerives: {
  int: JSONType<any, I, string>
  Date: JSONType<any, II, string>
}) => {
  const builderSchema = createBuilderSchema(fooDerives)
  return {
    fooCall: createJSONCall(builderSchema.calls.fooCall),
    barCall: createJSONCall(builderSchema.calls.barCall),
    bazCall: createJSONCall(builderSchema.calls.bazCall),
  }
}
