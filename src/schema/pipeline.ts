...
export type Pipe<I> = <P extends Produce, O = Output<P>>(produce: P) => Pipeline<I, O>
...
export const createPurePipeline = <I, O>(produce: Produce<I, O>): Pipeline<I, O> => {
  return {
    pipe: (nextProduce) => {
...
const alwaysTrue: Validate<any> = (input: any): input is Error => false as boolean

export const createPipeline = <I, O, E>(produce: ProduceE<I, O, E>, isE: Validate<E> = alwaysTrue): PipelineE<I, O, E> => {
  return {
    pipe: (nextProduce) => {
