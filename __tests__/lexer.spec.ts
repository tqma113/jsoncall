...
describe('lexer', () => {
  it('sample', () => {
    ...
    expect(lexer.next()).toMatchObject({ word: 'number', kind: 'primitivetype', range: { start: 12, end: 18, line: 1, column: 13 } })
    ...
    expect(lexer.next()).toMatchObject({ word: 'boolean', kind: 'primitivetype', range: { start: 31, end: 38, line: 2, column: 13 } })
    ...
    expect(lexer.next()).toMatchObject({ word: 'null', kind: 'primitivetype' })

    expect(lexer.next()).toMatchObject({ word: 'type', kind: 'keyword' })
    expect(lexer.next()).toMatchObject({ word: 'foo4', kind: 'name' })
    expect(lexer.next()).toMatchObject({ word: '=', kind: 'operator' })
    expect(lexer.next()).toMatchObject({ word: 'string', kind: 'primitivetype' })

    expect(lexer.next()).toMatchObject({ word: 'type', kind: 'keyword' })
    expect(lexer.next()).toMatchObject({ word: 'foo5', kind: 'name' })
    expect(lexer.next()).toMatchObject({ word: '=', kind: 'operator' })
    expect(lexer.next()).toMatchObject({ word: '[', kind: 'operator' })
    expect(lexer.next()).toMatchObject({ word: 'number', kind: 'primitivetype' })
    expect(lexer.next()).toMatchObject({ word: ']', kind: 'operator' })
    ...
    expect(lexer.next()).toMatchObject({ word: '=', kind: 'operator' })
    expect(lexer.next()).toMatchObject({ word: 'any', kind: 'specialtype' })

    expect(lexer.next()).toMatchObject({ word: 'type', kind: 'keyword' })
    expect(lexer.next()).toMatchObject({ word: 'foo9', kind: 'name' })
    expect(lexer.next()).toMatchObject({ word: '=', kind: 'operator' })
    expect(lexer.next()).toMatchObject({ word: 'none', kind: 'specialtype' })
